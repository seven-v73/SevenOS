...
status_json() {
  local contracts api bus_schema daemon daemon_src rust cargo events state
  contracts=0
  ...
  state="PLANNED"
  if [[ "$contracts" -ge 5 && "$api" == OK && "$bus_schema" == OK && "$daemon" == OK && "$daemon_src" == OK ]]; then
    state="FOUNDATION"
  fi
  ...
  CORE_STATE="$state" CONTRACTS="$contracts" API_STATE="$api" BUS_SCHEMA_STATE="$bus_schema" DAEMON_STATE="$daemon" DAEMON_SRC_STATE="$daemon_src" RUST_STATE="$rust" CARGO_STATE="$cargo" EVENT_COUNT="$events" EVENT_FILE="$EVENT_FILE" BUS_SCHEMA="$BUS_SCHEMA" python - <<'PY'
import json
import os
...
if status.get("state") in ("FOUNDATION", "READY_FOR_DAEMON"):
    add("daemon-service", "Prepare seven-daemon user service", "medium", "seven core doctor", "The next step is supervising the daemon through systemd user services.", "changes", "service")
...
doctor() {
  ...
  printf '===================\n'
  for path in scripts/state.sh scripts/control-plane.sh scripts/events.sh scripts/insights.sh scripts/phase-gate.sh server/seven-server.sh seven-core/README.md seven-core/bus-schema.json seven-core/daemon/Cargo.toml seven-core/daemon/src/main.rs; do
