...
calamares_hook_state() {
  local kind="$1"
  ROOT_DIR="$ROOT_DIR" KIND="$kind" python - <<'PY'
import os
...
if kind == "cleanup":
    checks = [
        ...
        ("archiso/profile/airootfs/root/customize_airootfs.sh", "Do not overwrite them here"),
    ]
    ...
    checks = [
        ...
        ("bin/seven-calamares-finalize", "run_optional_step"),
        ("bin/seven-calamares-finalize", "NetworkManager.service"),
        ("bin/seven-calamares-finalize", "sddm.service"),
    ]
    module = root / "installer/calamares/modules/shellprocess.conf"
    if not os.access(root / "bin/seven-calamares-finalize", os.X_OK):
        print("MISS")
        raise SystemExit(0)
else:
    print("MISS")
...
release_json() {
  ...
  calamares_livecleanup_state="$(calamares_hook_state cleanup)"
  calamares_postinstall_state="$(calamares_hook_state finalize)"
