...
release_json() {
  ...
  calamares_livecleanup_state="$([[ $(contains_state installer/calamares/settings.conf "shellprocess@livecleanup") == OK && $(contains_state installer/calamares/settings.conf "shellprocess-livecleanup.conf") == OK && $(contains_state installer/calamares/modules/shellprocess-livecleanup.conf "/bin/bash /opt/SevenOS/bin/seven-calamares-livecleanup") == OK && $(not_regex_state installer/calamares/modules/shellprocess-livecleanup.conf '\\$(log|hook|status)|\\$\\{(log|hook|status)\\}') == OK && $(contains_state bin/seven-calamares-livecleanup "without touching running live processes") == OK && $(contains_state bin/seven-calamares-livecleanup "target passwd file is missing") == OK && $(contains_state archiso/profile/airootfs/root/customize_airootfs.sh "seven-calamares-livecleanup") == OK ]] && printf OK || printf MISS)"
  calamares_postinstall_state="$([[ $(contains_state installer/calamares/modules/shellprocess.conf "/bin/bash /opt/SevenOS/bin/seven-calamares-finalize") == OK && $(not_regex_state installer/calamares/modules/shellprocess.conf '\\$(log|hook|status)|\\$\\{(log|hook|status)\\}') == OK && $(contains_state bin/seven-calamares-finalize "Clean live ISO residue") == OK && $(contains_state bin/seven-calamares-finalize "Configure network stack") == OK && $(contains_state bin/seven-calamares-finalize "Configure display stack") == OK && $(contains_state bin/seven-calamares-finalize "skipping network package installs during Calamares finalization") == OK && $(contains_state bin/seven-calamares-finalize "run_optional_step") == OK && $(contains_state bin/seven-calamares-finalize "NetworkManager.service") == OK && $(contains_state bin/seven-calamares-finalize "sddm.service") == OK && $(contains_state installer/calamares/settings.conf "- displaymanager") != OK && $(contains_state installer/calamares/settings.conf "- networkcfg") != OK && $(not_regex_state bin/seven-calamares-finalize 'install\\.sh"[[:space:]]+base|install\\.sh[[:space:]]+base') == OK && -x "$ROOT_DIR/bin/seven-calamares-finalize" ]] && printf OK || printf MISS)"
  ...
  live_portal_first_state="$([[ $(contains_state archiso/profile/airootfs/usr/local/bin/sevenos-live-ready "Opening Calamares directly for a stable graphical installation") == OK && $(contains_state archiso/profile/airootfs/usr/local/bin/sevenos-live-ready "open_calamares_direct") == OK && $(contains_state archiso/profile/airootfs/usr/local/bin/sevenos-live-ready "Calamares did not stabilize; trying the SevenOS portal fallback") == OK && $(contains_state archiso/profile/airootfs/usr/local/bin/sevenos-live-ready "open_sevenos_portal") == OK && $(contains_state archiso/profile/airootfs/usr/local/bin/sevenos-live-ready "confirm_installer_window") == OK && $(contains_state bin/seven-installer "focus_installer_window") == OK && $(contains_state bin/seven-installer "calamares-open.lock") == OK && $(contains_state bin/seven-installer "installer-portal-open.lock") == OK && $(contains_state archiso/profile/airootfs/usr/local/bin/sevenos-live-guard "installer_window_count") == OK ]] && printf OK || printf MISS)"
...
checks = [
    ...
    },
    {
        ...
        "state": os.environ["LIVE_PORTAL_FIRST_STATE"],
        "required": True,
        "title": "Live installer opens Calamares directly and keeps the SevenOS portal fallback",
