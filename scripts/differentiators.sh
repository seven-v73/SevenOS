...
ACTION="${1:-status}"
JSON_OUTPUT=0
REFRESH_CACHE="${SEVENOS_DIFFERENTIATORS_REFRESH:-0}"
CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/sevenos"
DIFFERENTIATORS_CACHE="$CACHE_DIR/differentiators-${ACTION}.json"
DIFFERENTIATORS_CACHE_TTL="${SEVENOS_DIFFERENTIATORS_CACHE_TTL:-300}"
...
if data.get("root") != str(Path(sys.argv[2]).resolve()):
    raise SystemExit(1)
if int(data.get("score", 0) or 0) < 80:
    raise SystemExit(1)
raise SystemExit(0)
PY
}
...
cached_payload_json() {
  local data
  if cache_is_fresh "$DIFFERENTIATORS_CACHE" "$DIFFERENTIATORS_CACHE_TTL"; then
    cat "$DIFFERENTIATORS_CACHE"
    return 0
  fi
  data="$(payload_json)"
  printf '%s\n' "$data" | write_json_cache "$DIFFERENTIATORS_CACHE" || true
  printf '%s\n' "$data"
}
