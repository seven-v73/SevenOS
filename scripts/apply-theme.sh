...
HYPR_THEME_ENV_CONFIG="$CONFIG_HOME/hypr/conf/sevenos-theme-env.conf"
SYSTEMD_USER_DIR="$CONFIG_HOME/systemd/user"
WAYLAND_SESSION_DIR="$DATA_HOME/wayland-sessions"
DESIGN_ENGINE_STATE="$SEVENOS_CONFIG_DIR/design-engine.json"
THEME_TRANSITION_FILE="$SEVENOS_CONFIG_DIR/theme-transition.json"
...
read_persisted_theme() {
  ...
  if [[ -f "$THEME_PREF" ]]; then
    persisted="$(awk -F= 'tolower($1) ~ /^(mode|theme_mode|sevenos_theme_mode)$/ {gsub(/[[:space:]"]/, "", $2); print tolower($2); exit}' "$THEME_PREF" 2>/dev/null || true)"
  fi
...
write_theme_transition() {
  ...
  if is_dry_run; then
    printf 'write %q with theme transition state=%q mode=%q stage=%q\n' "$THEME_TRANSITION_FILE" "$state" "$THEME_MODE" "$stage"
    return 0
  fi

  mkdir -p "$SEVENOS_CONFIG_DIR"
  STATE="$state" STAGE="$stage" MODE="$THEME_MODE" REQUESTED="${REQUESTED_THEME:-current}" NOW="$now" PID_VALUE="$$" TRANSITION_FILE="$THEME_TRANSITION_FILE" python - <<'PY'
import json
import os
from pathlib import Path

path = Path(os.environ["TRANSITION_FILE"])
payload = {
    "schema": "sevenos.theme-transition.v1",
    "state": os.environ.get("STATE", "unknown"),
    "stage": os.environ.get("STAGE", ""),
    "mode": os.environ.get("MODE", "dark"),
    "requested": os.environ.get("REQUESTED", "current"),
...
if payload["state"] == "running":
    ...
    try:
        ...
        existing = {}
    payload["started_at"] = existing.get("started_at") if existing.get("state") == "running" else payload["updated_at"]
else:
...
mark_theme_failed_on_exit() {
  local status=$?
  if [[ "$THEME_TRANSITION_ACTIVE" == "1" && "$status" != "0" ]]; then
    write_theme_transition "failed" "interrupted" || true
  fi
  exit "$status"
}
...
sync_theme_environment() {
  log_info "Synchronizing SevenOS theme mode with the user session..."

  if is_dry_run; then
    printf 'export SEVENOS_THEME_MODE=%q\n' "$THEME_MODE"
    printf 'systemctl --user import-environment SEVENOS_THEME_MODE GTK_THEME QT_QPA_PLATFORMTHEME QT_STYLE_OVERRIDE ELECTRON_OZONE_PLATFORM_HINT\n'
    printf 'dbus-update-activation-environment --systemd SEVENOS_THEME_MODE GTK_THEME QT_QPA_PLATFORMTHEME QT_STYLE_OVERRIDE ELECTRON_OZONE_PLATFORM_HINT\n'
    return 0
  fi
...
write_hyprland_theme_environment() {
  log_info "Writing SevenOS Hyprland theme environment..."

  if is_dry_run; then
    printf 'write %q with light/dark env for Hyprland-launched apps\n' "$HYPR_THEME_ENV_CONFIG"
    return 0
  fi

  mkdir -p "$(dirname -- "$HYPR_THEME_ENV_CONFIG")"
  {
    printf '# Generated by SevenOS. Do not edit by hand; use Settings > Appearance.\n'
    printf 'env = SEVENOS_THEME_MODE,%s\n' "$THEME_MODE"
    printf 'env = GTK_THEME,%s\n' "$GTK_THEME"
    printf 'env = QT_QPA_PLATFORMTHEME,qt6ct\n'
    printf 'env = QT_STYLE_OVERRIDE,kvantum\n'
    printf 'env = XDG_CURRENT_DESKTOP,Hyprland\n'
    printf 'env = XDG_SESSION_TYPE,wayland\n'
    printf 'env = ELECTRON_OZONE_PLATFORM_HINT,auto\n'
    if [[ "$THEME_MODE" == "light" ]]; then
      printf 'env = SEVENOS_COLOR_SCHEME,prefer-light\n'
    else
      printf 'env = SEVENOS_COLOR_SCHEME,prefer-dark\n'
    fi
  } > "$HYPR_THEME_ENV_CONFIG"
}
  ...
  if [[ -n "${WAYLAND_DISPLAY:-}" ]]; then
    if command -v systemctl >/dev/null 2>&1; then
      ...
      systemctl --user try-restart \
        ...
        sevenos-shell-experience.service \
...
configure_user_session_services() {
  ...
  if is_dry_run; then
    ...
    printf 'systemctl --user enable sevenos-session.target sevenos-theme-session.service\n'
    return 0
  fi
  ...
  if command -v systemctl >/dev/null 2>&1; then
    ...
    systemctl --user enable sevenos-theme-session.service sevenos-waybar-context.service sevenos-waybar.service sevenos-notifications.service sevenos-wallpaper.service sevenos-idle.service sevenos-shell-experience.service >/dev/null 2>&1 || true
  fi
}
...
sync_theme_environment
normalize_profile_theme_modes
write_theme_transition "running" "surfaces"
...
write_theme_transition "running" "identity"
...
run_cmd cp "$ROOT_DIR/identity/assets/symbol-seven-prism.svg" "$DATA_HOME/icons/hicolor/scalable/apps/sevenos-prism.svg"
run_cmd cp "$ROOT_DIR/identity/assets/symbol-seven-prism-mark.svg" "$DATA_HOME/sevenos/identity/symbol-seven-prism-mark.svg"
...
restore_persisted_wallpaper
write_hyprpaper_config
write_theme_transition "running" "palette"
if is_dry_run; then
...
write_theme_transition "running" "reload"
reload_desktop_session

write_theme_transition "success" "complete"
THEME_TRANSITION_ACTIVE=0
