...
gates = [
    ...
    },
    {
        ...
        "state": "PASS" if server_service in ("RUN", "OK") and all(server_deps.get(key) == "OK" for key in ("go", "podman", "caddy")) else "WARN",
        "actual": server_service,
        "target": "RUN",
        "command": "seven improve deployment --apply --yes",
        "reason": "Seven Hub and future Shell need a reliable local backend.",
    },
    {
        "key": "installer",
