...
SEVENOS_DRY_RUN=1 "$ROOT_DIR/scripts/identity.sh" status >/dev/null
...
SEVENOS_UPDATE_FAST=1 SEVENOS_HEALTH_FAST=1 SEVENOS_DISTRIBUTION_FAST=1 SEVENOS_DRY_RUN=0 "$ROOT_DIR/bin/seven" state --json | python -c 'import json,sys; data=json.load(sys.stdin); required={"packages_strategy","packages_catalog","packages_footprint"}; catalog=data.get("packages_catalog") or {}; ok=required.issubset(data) and catalog.get("schema")=="sevenos.app-catalog.v1" and int(catalog.get("count",0) or 0) >= 12; raise SystemExit(0 if ok else 1)'
...
SEVENOS_DRY_RUN=1 "$ROOT_DIR/bin/seven" --dry-run session start >/dev/null
...
SEVENOS_DRY_RUN=1 "$ROOT_DIR/bin/seven" --dry-run shell plan --json >/dev/null
SEVENOS_DRY_RUN=1 "$ROOT_DIR/bin/seven" --dry-run shell preview >/dev/null
SEVENOS_DRY_RUN=1 "$ROOT_DIR/bin/seven" --dry-run shell doctor >/dev/null
...
SEVENOS_DRY_RUN=1 "$ROOT_DIR/install.sh" blackarch-full --dry-run >/dev/null
SEVENOS_DRY_RUN=1 "$ROOT_DIR/bin/seven" shield toolchain blackarch-full --dry-run >/dev/null
