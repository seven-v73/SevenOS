...
fn daemon_phase_gate_json() {
    ...
    let server_percent = if server_service == "RUN" {
        100
    ...
    let gates = vec![
        ...
        phase_gate_item(
            ...
            if server_service == "RUN" || server_service == "READY" {
                "PASS"
            } else {
                "BLOCK"
            },
            ...
            server_service,
            "seven server plan",
            "The ecosystem needs a local OS API surface before Phase 5 work can be credible.",
        ),
        phase_gate_item(
            "installer",
            "Installer path",
